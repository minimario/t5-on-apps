...
class APPSBaseDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, max_tokens, tokenizer_path, mode, answer_type_preprocessing):
        self.dataset = dataset
        self.max_tokens = max_tokens
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path)
        if "/" in tokenizer_path:
            self.tokenizer_path = tokenizer_path.split("/")[-1]
        self.mode = mode
        self.answer_type_preprocessing = answer_type_preprocessing
        self.samples = []  # Should be set in initialize()
        self.initialize(self.tokenizer)

    def initialize(self, tokenizer):
        try:
            with open(f"cache/cache-{self.max_tokens}-{self.mode}-{self.tokenizer_path}-new.json", "r") as f:
                samples = json.load(f)
                self.samples = samples["all_samples"]
                self.samples_dict = samples["all_samples_dict"]
                print(f"Loaded {len(self.samples)} samples from cache")
                return
        except:
            print(f"cache-{self.max_tokens}-{self.mode}-{self.tokenizer_path}.json not found.")
            pass
        ...
        for idx in tqdm(range(len(self.dataset))):
            ...
            try:
                ...
                continue
            # starter code
            if self.answer_type_preprocessing = "loubna":
                try:
        ...
        # Cached samples
        print("Caching")
        samples = {"all_samples": all_samples, "all_samples_dict": all_samples_dict}
        with open(f"cache/cache-{self.max_tokens}-{self.mode}-{self.tokenizer_path}-new.json", "w") as f:
            json.dump(samples, f)
